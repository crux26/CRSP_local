...
proc sql;
create table spxcall_mnth_2nd_only as
...
/**/
proc sql;
create table spxcall_mnth_2nd_only_2 as
select distinct a.*, a.datedif as datedif_2nd_excluded
from spxcall_mnth_2nd_only as a,
spxcall_mnth_2nd_only as b
...
proc sql;
create table spxcall_mnth_2nd as
select distinct a.*, min(b.datedif) as min_datedif_2nd
from spxcall_mnth_2nd_only as a
left join
spxcall_mnth_2nd_only_2 as b
...
proc sql;
create table spxput_mnth_2nd_only as
select a.*, min(datedif) as min_datedif
from
spxput_mnth as a
group by date
...
proc sql;
create table spxput_mnth_2nd_only_2 as
select distinct a.*, a.datedif as datedif_2nd_excluded
from
spxput_mnth_2nd_only as a,
spxput_mnth_2nd_only as b
where a.date=b.date and a.strike_price=b.strike_price
group by a.date
having a.datedif~=b.min_datedif
order by a.date, a.datedif, a.strike_price;
...
proc sql;
create table spxput_mnth_2nd as
select distinct a.*, min(b.datedif) as min_datedif_2nd
from
spxput_mnth_2nd_only as a
left join
spxput_mnth_2nd_only_2 as b
on a.date=b.date and a.symbol=b.symbol
group by a.date
order by a.date, a.datedif, a.strike_price;
quit;
...
proc datasets lib=work nolist;
	delete spxcall_mnth_2nd_only spxcall_mnth_2nd_only_2 spxput_mnth_2nd_only spxput_mnth_2nd_only_2;
run;
quit;

data myoption.spxcall_mnth_2nd; set spxcall_mnth_2nd; run;
data myoption.spxput_mnth_2nd; set spxput_mnth_2nd; run;
...
proc export data = spxcall_mnth_2nd
outfile = "D:\Dropbox\GitHub\TRP\data\rawdata\SPXCall_Mnth_2nd.xlsx"
DBMS = xlsx REPLACE;
run;

proc export data = spxput_mnth_2nd
