...
data BEM.MktCap; set BEM.msf2(keep=&vars year month mktcap) ; run;

proc sort data=BEM.MktCap; by permno year; run;

/* Keep June's MktCap constant until the following May. */
data BEM.MktCap2(drop=count _MktCap JuneCount cum_JuneCount); set BEM.MktCap;
by permno year;
...
proc sort data=BEM.MktCap;
by year month permno;
run;

%include myMacro('SummRegResult_custom.sas');
%SummRegResult_custom(data=BEM.MktCap, out=BEM.MktCapPrdcStat, var=MktCap, by=year);

%include myMacro('Trans.sas');
%Trans(data=BEM.MktCapPrdcStat, out=BEM.MktCapPrdcStat, var=MktCap, id=_STAT_, by=year );

/*Re-order variables.*/
data BEM.MktCapPrdcStat;
retain year coeff mean StdDev Skew Kurt Min p5 p25 median p75 p95 max n;
set BEM.MktCapPrdcStat;
run;

%include myMacro('ObsAvg.sas');
%ObsAvg(data=BEM.MktCapPrdcStat, out=BEM.MktCapAvgStat, by=coeff, drop=_TYPE_ _FREQ_ year);

/*Re-order variables.*/
data BEM.MktCapAvgStat;
retain year coeff mean StdDev Skew Kurt Min p5 p25 median p75 p95 max n;
set BEM.MktCapAvgStat;
run;
