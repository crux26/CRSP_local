%macro tsreg_exret_TailRV_Lin(data=, keyvar=, DepVarList=, IndepVarList=, out_prefix=, lag=);
	...
	%do i=1 %to &nwords;
		...
		%do j=1 %to &nwords_;
			...
			proc model data=&data.;
				parms a b c;
/*				exogenous &DepVar. &IndepVar. &keyvar.; *VRP=RV - lag(VIX);*/
				exogenous &IndepVar. &keyvar.; *VRP=RV - lag(VIX);
