...
proc sort data=crsp_m2;
	by date permco MEq;
run;

data crsp_m2a (drop=MEq);
	set crsp_m2;
	by date permco MEq;
	retain ME;
...
proc sort data=crsp_m2a nodupkey;
	by permno date;
run;

data crsp_m3 DECME(rename=(ME=DEC_ME));
	set crsp_m2a;
	by permno date;
...
data indadjbm_dt_june;
	set indadjbm_dt;

	if month(datadate) ^=6 then
		delete;
run;
...
data tmp_1;
	...
	if bm_crsp>0 and me>0 and count>=2 then
		do;
			positivebeme=1;
			...
			if 0< bm_crsp <= beme30 then
				btmport = 'L';
			else if beme30 < bm_crsp <= beme70 then
				btmport = 'M';
			else if bm_crsp > beme70 then
				btmport = 'H';
			else btmport =' ';
		end;
	else positivebeme=0;
...
data mysas.ff_nfirms_DT;
	set ff_nfirms_DT;
run;

data mysas.ff_factors_DT;
	set ff_factors_DT;
run;

data mysas.ff_vwret_DT;
	set vwret_DT;
run;
