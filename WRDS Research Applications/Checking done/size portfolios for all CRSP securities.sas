...
data msfx1    (keep=date permno size year_prev ret size_lag)
	...
	where also nmiss(prc,shrout,ret)=0;
...
** For the GPLOT output below **;
options orientation=landscape;

** Do not print the date or page number **;
options nodate nonumber;

** Use "pdf color" as "Device" for graphics output **;
options device=pdfc;

** Activate PDF output, specify destination **;
ods pdf file='./ps_graph.pdf';

** Turn off sasgraph.pdf, now superseded by ps_graph.pdf **;
ods listing close;
