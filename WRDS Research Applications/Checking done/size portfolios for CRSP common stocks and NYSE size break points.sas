...
%let evars = permco shrcd exchcd dlret;

/* Invoke CRSPMERGE WRDS Research Macro. Data Output: CRSP_M */
%crspmerge(s=m,start=&begdate,end=&enddate,sfvars=&fvars,sevars=&evars,filters=&filtr,outset=crsp_m );
...
data x2;
	...
	if size <= 0 then
		group =.;
	else if size >  0     and size < dec10 then
		group =1;
	else if size >= dec10 and size < dec20 then
		group =2;
	else if size >= dec20 and size < dec30 then
		group =3;
	else if size >= dec30 and size < dec40 then
		group =4;
	else if size >= dec40 and size < dec50 then
		group =5;
	else if size >= dec50 and size < dec60 then
		group =6;
	else if size >= dec60 and size < dec70 then
		group =7;
	else if size >= dec70 and size < dec80 then
		group =8;
	else if size >= dec80 and size < dec90 then
		group =9;
	else if size >= dec90                  then
		group =10;
	label group = 'DECILE';
...
/*INTNX('QTR', date, 1): [Jan~Mar] as the same quarter; NOT adding 3 months. */
/*b.date: preceding quarter, a.date: following quarter.*/
proc sql;
	create table msfx1
		as select a.*, b.group
