...
proc sort data=crspm2a nodupkey;
	by permno date;
run;
...
/* RET_PF(t-1) = ME(t-2) * (1 + CUMRETX (June,t-1)) <-- "t" is an index of month */
...
data ccm1a (label="COMPUSTAT & ccmxpf_linktable only");
	set ccm1;
...
data ccm2a (label="COMPUSTAT & ccmxpf_linktable only");
	set ccm1a;
	by permno year datadate;
...
data june;
	...
	If beme>0 and me>0 and count>=2 then
		do;
			...
			* and BE (t-1) >0 and more than two years in Compustat ("count>=2");
			...
			if 0 < beme <= beme30 then
				btmport = 'L';
			else if beme30 < beme <= beme70 then
				btmport = 'M';
			else if beme  > beme70 then
				btmport = 'H';
	...
	if cmiss(sizeport,btmport)=0 then
		nonmissport=1;
	else nonmissport=0;
