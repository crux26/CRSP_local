...
/*				%let idate2= %sysfunc(intnx(&freq, &idate1, &n, end));*/
				...
				* Add loop date range variables to output set;
				...
				data _outest_ds_final;
					...
					win_beg = &idate1;
                    win_end = &idate2;
					date1 = max(&idate1, date1_min, &sdate1);
					date2 = min(&idate2, &sdate2, date2_max);
/*					date1 = max(&idate1, date1_min);*/
/*					date2 = min(&idate2, date2_max);*/
					by permno;
	...
	* end for date check pass section;
	proc datasets lib=work nolist;
		delete tmp _all_ds _outest_ds _outest_ds_ _outest_ds_final date2_max;
	quit;
