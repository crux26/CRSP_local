...
/******************************************************************************************/
/*   Probit with two-way cluster-robust SEs, z-stats, and p-values  */
/******************************************************************************************/

%MACRO clus2probit(data, out, yvar, xvars, cluster1, cluster2);
    options nonotes nosource;
    ods listing close;
    ods exclude all;
    ods graphics off;
	/* do interesection cluster*/
	proc surveylogistic data=&data; class &yvar /descending; cluster &cluster1 &cluster2; model &yvar= &xvars /link=probit covb; ods output CovB = CovI; quit;
	/* Do first cluster */
	proc surveylogistic data=&data; class &yvar /descending; cluster &cluster1; model &yvar= &xvars /link=probit covb; ods output CovB = Cov1; quit;
	/* Do second cluster */
	proc surveylogistic data=&data; class &yvar /descending; cluster &cluster2; model &yvar= &xvars /link=probit covb; ods output CovB = Cov2 ParameterEstimates = params (drop=df); quit;
...
%clus2probit(data=ds_in, out=clus2probit, yvar=dependentvariable, xvars=listofindependentvariables, cluster1=gvkey, cluster2=fyear);
