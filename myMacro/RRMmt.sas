%macro RRMmt(data= , out_ds= , id= , date=date, statList=mean stddev var skew kurt n,
...
/*=====================================================================================*/
				ods results off;
				proc means data=tmp StackOdsOutput &statList.;
					var resid;
					where &date between &idate1 and %sysfunc(min(&idate2,&sdate2));
					&by_id;
					ods output summary=_outest_ds;
				run;
/*=====================================================================================*/
				...
				* Append results;
				proc datasets nolist;
					append base=_all_ds data=_outest_ds_final(where=(N>=&minwin));
