/*Takes ages to be done. crsp_d(obs=24575) takes about 18m, step=1D. */
...
%MACRO IDVOL (INSET=, OUTSET=, DATEVAR=, RETVAR=, FREQ=, WINDOW=,STEP=, MIN=, MODEL=);
    ...
    %if &model=m %then
        %let vars=mktrf;
    %else %if &model=ff  %then
        %let vars=mktrf smb hml;
    %else %if &model=ffm %then
        %let vars=mktrf smb hml umd;
    ...
    %do j=&start %to &finish %by &step;
        ...
        data _null_;
            set _caldates (sortedby=n where=(n=&j));
        ...
        proc append base=_idvol data=_stats force;
        run;
