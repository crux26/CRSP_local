...
/**************************************************************************************/
  ...
  ** First make sure the calendar file is long enough to accomodate at least
      one window >= MINSIZE.  This step may reset SIZE to a smaller number, or
      to 0 (no acceptable windows are possible) **;
  data _null_;
/*    set mysas.&freq.si nobs=ndates;  ** NDATES gets number of dates in this calendar **;*/
	set mysas.&freq.sia nobs=ndates;  ** NDATES gets number of dates in this calendar **;
    max_possible_size=ndates - 1;   ** Max possible windows size this calendar    **;
      if max_possible_size >= &size then ;  ** No adjustment to SIZE needed         **;
        ...
        if max_possible_size < &minsize then do;  ** No acceptable window possible  **;
          put '** ****************************************************** **' /
            "** Largest trading-date window size in mysas.&freq.sia is " max_possible_size  @59 '**' /
        ...
        put '** ****************************************************** **' /
            "** Largest trading-date window size in mysas.&freq.si is " max_possible_size  @59 '**' /
