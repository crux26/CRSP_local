...
/*-----------------------------------------------USE---------------------------------------------------*/
/*%varnames(dsid=mysas.mseall, varlist=msevars) ;*/
/*%varnames(dsid=mysas.msf, varlist=msfvars) ;*/
/*%CRSPMERGE2(s=m,START=01JAN1959, END=31DEC2016, SFVARS=&msfvars, SEVARS=&msevars, OUTSET=crsp_&s.2);*/
/*------------------------------------------------------------------------------------------------------*/
...
/* &eventvars, expanded the "size" of &eventvars. */
...
/*drop &sevars_l ;*/
/* Don't see why, but above DROP statement complains for WARNING - The variable xxx  */
/* in the DROP, KEEP, or RENAME list has never been referenced. */
/* This only happens in CRSPMERGE2(), NOT in CRSPMERGE. */
/* --> This is because %eventvars is a superset of the default &sevars in CRSPMERGE(), */
/* while %eventvars is a subset of &sevars in CRSPMERGE2(). */
run;
/*--------------------------------------------------------------------------------*/
