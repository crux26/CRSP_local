...
%macro SummRegResult_custom(data=, out=, var=, by=);
    %if %sysfunc(findw("&data", '.'))=0 %then
        %let lib_data = work;
    %else %let lib_data = %sysfunc(scan(&data, 1, '.'));
    ...
    data tmp1;
        ...
        if findw('&by', month, 'i') >1 & month<=9 then
            month = cats(0,month);

        /*'i': ignores character case. */
        if findw('&by', day, 'i') > 1 & day<=9 then
            day = cats(0,day);
    ...
    data _null_;
        ...
        if month=. then
            call symputx('drop1','month');
        else call symputx('drop1','');

        if day=. then
            call symputx('drop2', 'day');
