...
/*             - LAGS:     number of LAGs to be used in the Newey-West adjustments    */
/* ********************************************************************************* */
%MACRO FM(DATA=, OUT=, DATEVAR=, byvar=, DEPVAR=, INDVARS=, LAGS=);
	...
	proc reg data=_temp outest=_results noprint;
		by &datevar &byvar.;
		model &depvar=&indvars / edf adjrsq;
	run;
	...
	data &OUT;
		...
		label Rsq='R-squared';
		label adjRsq='Adjusted R-squared';
	run;

	%put ### COMPUTING FAMA-MACBETH COEFFICIENTS...;

	/*--------------------------START OF THE DO LOOP------------------------*/
    %let indvars = intercept &indvars.; /*Add intercept for FM result.*/
	%do k=1 %to %nwords(&indvars); /* DO loop up to #(&indvars) */
		...
		proc means data=_results mean;
			var _EDF_ _RSQ_ _ADJRSQ_;
			ods output summary=_RSQ(keep=_EDF__mean _RSQ__mean _ADJRSQ__mean);
		run;
		...
		proc model data=_results; /* PROC MODEL: OLS, 2SLS, 3SLS, GMM, SMM, MC simulation, ... */
			...
			fit &var/gmm kernel=(bart,%eval(&LAGS+1),0); /* KERNEL: PARZEN, BART, QS. Default: (PARZEN, 1, 0.2). */
			by &byvar.;
		...
		data _params (drop=&var._n);
			merge _params
				...
				_RSQ(rename=(_EDF__mean=n _RSQ__mean=RSQ _ADJRSQ__mean=ADJRSQ));
			;
	...
	/*house cleaning */
	proc sql;
		drop table _temp, _params, _results, _uncorr, _RSQ;
	quit;
