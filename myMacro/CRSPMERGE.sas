/*Using below results in FUCKING ERRORONEOUS RESULT.*/
/*	%let sdate = %sysfunc(putn("&start"d,5.));*/
/*	%let edate = %sysfunc(putn("&end"d,5.));*/

/*EXCLUDING PERMCO IN SEVARS YIELDS NO OBS: include it!*/
/*--> In the code, there is "if missing(permco) then delete"*/
...
/* ********************************************************************************* */
%MACRO CRSPMERGE (s=m, START='01JAN2000'd, END='30JUN2001'd, SFVARS=vol prc ret shrout,
	...
	%let sf       = a_stock.&s.sf;
	%let se       = a_stock.&s.seall;
	%let senames  = a_stock.&s.senames;
